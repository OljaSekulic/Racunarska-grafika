                ...
                foreach (int meshIndex in node.MeshIndices)
                {
                    ...
                    foreach (Assimp.Face face in mesh.Faces)
                    {
                        ...
                        for (int i = 0; i < face.IndexCount; i++)
                        {
                            ...
                            if (hasColors)
                                gl.Color(mesh.GetVertexColors(10)[vertexIndex].R, mesh.GetVertexColors(0)[vertexIndex].G, mesh.GetVertexColors(0)[vertexIndex].B, mesh.GetVertexColors(0)[vertexIndex].A);
                            else
                            {
                                // Permutacija boje poligona u zavisnosti od parnosti indeksa
                             
                                if (vertexIndex % 2 == 0)
                                    gl.Color(20f, 0.3f, 0.3f);
                                else
                                    gl.Color(20f, 0.4f, 0.4f);
                            }
